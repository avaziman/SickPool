...
#define posix_memalign(p, a, s) \
    ...
    enum
    {
        ...
        // Any excess over a power of 2 will not get mutated, and any excess
        // over
        // power of 2 + Haraka sized key will not be used
        VERUSKEYSIZE = 1024 * 8 + (40 * 16),
        SOLUTION_VERUSHHASH_V2 =
            1,  // this must be in sync with CScript::SOLUTION_VERUSV2
        SOLUTION_VERUSHHASH_V2_1 =
            3,  // this must be in sync with CScript::ACTIVATE_VERUSHASH2_1
        SOLUTION_VERUSHHASH_V2_2 =
            4  // this must be in sync with CScript::ACTIVATE_VERUSHASH2_2
    };
    ...
    if (__cpuverusoptimized & 0x80)
    {
        ...
        {
            __cpuverusoptimized = ((ecx & (bit_AVX | bit_AES | bit_PCLMUL)) ==
                                   (bit_AVX | bit_AES | bit_PCLMUL));
        }
    }
#endif
...
struct verusclhasher
{
    ...
    uint64_t keyMask;
    uint64_t (*verusclhashfunction)(void *random, const unsigned char buf[64],
                                    uint64_t keyMask, __m128i **pMoveScratch);
    __m128i (*verusinternalclhashfunction)(__m128i *randomsource,
                                           const __m128i buf[4],
                                           uint64_t keyMask,
                                           __m128i **pMoveScratch);
    ...
    verusclhasher(uint64_t keysize = VERUSKEYSIZE,
                  int solutionVersion = SOLUTION_VERUSHHASH_V2)
        : keySizeInBytes((keysize >> 5) << 5)
    {
#ifdef __APPLE__
        __tls_init();
#endif
        if (IsCPUVerusOptimized())
        {
            if (solutionVersion >= SOLUTION_VERUSHHASH_V2_1)
            {
                if (solutionVersion >= SOLUTION_VERUSHHASH_V2_2)
                {
                    ...
                    verusinternalclhashfunction =
                        &__verusclmulwithoutreduction64alignedrepeat_sv2_2;
                }
                else
                {
                    verusclhashfunction = &verusclhash_sv2_1;
                    verusinternalclhashfunction =
                        &__verusclmulwithoutreduction64alignedrepeat_sv2_1;
                }
            }
            else
            {
                ...
                verusinternalclhashfunction =
                    &__verusclmulwithoutreduction64alignedrepeat;
            }
        }
        else
        {
            if (solutionVersion >= SOLUTION_VERUSHHASH_V2_1)
            {
                if (solutionVersion >= SOLUTION_VERUSHHASH_V2_2)
                {
                    ...
                    verusinternalclhashfunction =
                        &__verusclmulwithoutreduction64alignedrepeat_sv2_2_port;
                }
                else
                {
                    verusclhashfunction = &verusclhash_sv2_1_port;
                    verusinternalclhashfunction =
                        &__verusclmulwithoutreduction64alignedrepeat_sv2_1_port;
                }
            }
            else
            {
        ...
        if (verusclhasher_key.get() &&
            keySizeInBytes != ((verusclhash_descr *)verusclhasher_descr.get())
                                  ->keySizeInBytes)
        {
        ...
        if (!(key = verusclhasher_key.get()) &&
            (verusclhasher_key.reset(
                 (unsigned char *)alloc_aligned_buffer(keySizeInBytes << 1)),
             key = verusclhasher_key.get()))
        {
            verusclhash_descr *pdesc;
            if (verusclhasher_descr.reset(new verusclhash_descr()),
                pdesc = (verusclhash_descr *)verusclhasher_descr.get())
            {
    ...
    {
        verusclhash_descr *pdesc =
            (verusclhash_descr *)verusclhasher_descr.get();
        return (unsigned char *)verusclhasher_key.get() + pdesc->keySizeInBytes;
    }

    // returns a per thread, writeable scratch pad that has enough space to hold
    // a pointer for each mutated entry in the refresh hash
    ...
    inline uint64_t keyrefreshsize() const { return keyMask + 1; }
    ...
    inline void *fixupkey(void *hashKey, verusclhash_descr &desc)
    {
        ...
        __m128i **ppfixup = getpmovescratch(
            ret + desc.keySizeInBytes);  // past the part to refresh from
        for (__m128i *pfixup = *ppfixup; pfixup; pfixup = *++ppfixup)
        {
            *pfixup = *(pfixup + ofs);  // we hope the compiler cancels this
                                        // operation out before add
        }
        return hashKey;
    }

    // this prepares a key for hashing and mutation by copying it from the
    // original key for this block WARNING!! this does not check for NULL ptr,
    // so make sure the buffer is allocated
    ...
    inline uint64_t operator()(const unsigned char buf[64]) const
    {
        ...
        verusclhash_descr *pdesc =
            (verusclhash_descr *)verusclhasher_descr.get();
        return (*verusclhashfunction)(
            pkey, buf, keyMask,
            (__m128i **)(pkey + (pdesc->keySizeInBytes + keyrefreshsize())));
    }
    ...
    inline uint64_t operator()(const unsigned char buf[64], void *pkey) const
    {
        ...
        return (*verusclhashfunction)(
            pkey, buf, keyMask,
            (__m128i **)((unsigned char *)pkey +
                         (pdesc->keySizeInBytes + keyrefreshsize())));
    }

    inline uint64_t operator()(const unsigned char buf[64], void *pkey,
                               __m128i **pMoveScratch) const
    {
        return (*verusclhashfunction)((unsigned char *)pkey, buf, keyMask,
                                      pMoveScratch);
    }
};
