 ...
 * Distributed under the Apache 2.0 software license, available in the original
 *form for clhash here:
 *https://github.com/lemire/clhash/commit/934da700a2a54d8202929a826e2763831bd43cf7#diff-9879d6db96fd29134fc802214163b95a
 *
 * Original CLHash code and any portions herein, (C) 2017, 2018 Daniel Lemire
 *and Owen Kaser Faster 64-bit universal hashing using carry-less
 *multiplications, Journal of Cryptographic Engineering (to appear)
 *
 * Best used on recent x64 processors (Haswell or better).
 *
 * This implements an intermediate step in the last part of a Verus block hash.
 *The intent of this step is to more effectively equalize FPGAs over GPUs and
 *CPUs.
 *
 **/
#include <assert.h>
...
// attempt to workaround horrible mingw/gcc destructor bug on Windows and Mac,
// which passes garbage in the this pointer we use the opportunity of control
// here to clean up all of our tls variables. we could keep a list, but this is
// a safe, functional hack
...
#if defined(__arm__) || \
    defined(__aarch64__)  // intrinsics not defined in SSE2NEON.h

static inline __attribute__((always_inline)) __m128i _mm_set_epi64x(uint64_t hi,
                                                                    uint64_t lo)
{
    __m128i result;
...
static inline __attribute__((always_inline)) __m128i _mm_mulhrs_epi16(
    __m128i _a, __m128i _b)
{
    int16_t result[8];
    ...
    {
        result[i] =
            (int16_t)((((int32_t)(a[i]) * (int32_t)(b[i])) + 0x4000) >> 15);
    }
...
static inline __attribute__((always_inline)) uint8x16_t _mm_aesenc_si128(
    uint8x16_t a, uint8x16_t RoundKey)
{
    return vaesmcq_u8(vaeseq_u8(a, (uint8x16_t){})) ^ RoundKey;
}

static inline __attribute__((always_inline)) __m128i _mm_clmulepi64_si128(
    const __m128i a, const __m128i &b, int imm)
{
    return (__m128i)vmull_p64(vgetq_lane_u64(a, 1), vgetq_lane_u64(b, 0));
}

__m128i _mm_setr_epi8(u_char c0, u_char c1, u_char c2, u_char c3, u_char c4,
...
int64_t _mm_cvtsi128_si64(__m128i a) { return ((int64_t *)&a)[0]; }
...
static inline __attribute__((always_inline)) __m128i lazyLengthHash(
...
// precompReduction64
static inline __attribute__((always_inline)) __m128i precompReduction64_si128(
    __m128i A)
{
    ...
    __m128i Q3 = _mm_shuffle_epi8(
        _mm_setr_epi8(0, 27, 54, 45, 108, 119, 90, 65, (char)216, (char)195,
...
static inline __attribute__((always_inline)) uint64_t precompReduction64(
    __m128i A)
{
    return _mm_cvtsi128_si64(precompReduction64_si128(A));
}

static inline __attribute__((always_inline)) void fixupkey(
...
static inline __attribute__((always_inline)) void haraka512_keyed_local(
    unsigned char *out, const unsigned char *in, const u128 *rc)
{
    u128 s[4], tmp;
                                                    ...
                                                    __m128i **pMoveScratch)
{
    ...
    // to work with this algorithm. we take the value from the last element
    // inside the keyMask + 2, as that will never be used to xor into the
    // accumulator before it is hashed with other values first
    ...
    for (int64_t i = 0; i < 32; i++)
    {
        ...
        // get two random locations in the key, which will be mutated and
        // swapped
        ...
        *(pMoveScratch++) = prandex;
        ...
        switch (selector & 0x1c)
        {
            case 0:
            {
                ...
                const __m128i temp2 =
                    _mm_load_si128(pbuf - (((selector & 1) << 1) - 1));
                ...
                const __m128i clprod12 =
                    _mm_clmulepi64_si128(add12, add12, 0x10);
            ...
            case 4:
            {
                ...
                const __m128i clprod1 = _mm_clmulepi64_si128(add1, add1, 0x10);
                acc = _mm_xor_si128(clprod1, acc);
                const __m128i clprod2 =
                    _mm_clmulepi64_si128(temp2, temp2, 0x10);
            ...
            case 8:
            {
                ...
                const __m128i clprod12 =
                    _mm_clmulepi64_si128(add12, add12, 0x10);
                acc = _mm_xor_si128(clprod12, acc);
                const __m128i clprod22 =
                    _mm_clmulepi64_si128(temp22, temp22, 0x10);
            ...
            case 0x14:
            {
                ...
                uint64_t rounds =
                    selector >> 61;  // loop randomly between 1 and 8 times
                __m128i *rc = prand;
                ...
                do
                {
                    if (selector & (0x10000000 << rounds))
                    {
                        ...
                        const __m128i temp2 =
                            _mm_load_si128(rounds & 1 ? pbuf : buftmp);
                    ...
                    else
                    {
                        onekey = _mm_load_si128(rc++);
                        __m128i temp2 =
                            _mm_load_si128(rounds & 1 ? buftmp : pbuf);
    ...
    for (int64_t i = 0; i < 32; i++)
    {
        ...
        *(pMoveScratch++) = prandex;
        ...
        switch (selector & 0x1c)
        {
            ...
            {
                // we'll just call this one the monkins loop, inspired by Chris
                // - modified to cast to uint64_t on shift for more variability
                // in the loop
                const __m128i *buftmp = pbuf - (((selector & 1) << 1) - 1);
                __m128i tmp;  // used by MIX2
            ...
            case 0x18:
            {
                ...
                do
                {
                    if (selector & (((uint64_t)0x10000000) << rounds))
                    {
                        ...
                        const __m128i modulo =
                            _mm_cvtsi32_si128(dividend % divisor);
                        acc = _mm_xor_si128(modulo, acc);
                    }
                    else
                    {
