...
inline bool parseWidthOrPrecision(int& n, const char*& c, bool positionalMode,
                                  const detail::FormatArg* args, int& argIndex,
                                  int numArgs)
{
    if (*c >= '0' && *c <= '9')
    {
    ...
    else if (*c == '*')
    {
        ...
        if (positionalMode)
        {
            ...
            if (*c != '$')
                TINYFORMAT_ERROR(
                    "tinyformat: Non-positional argument used after a "
                    "positional one");
                                         ...
                                         int& argIndex, int numArgs)
{
    ...
    if (*c >= '0' && *c <= '9')
    {
        ...
        {
            TINYFORMAT_ERROR(
                "tinyformat: Non-positional argument used after a positional "
                "one");
        }
        else
        {
                       ...
                       const detail::FormatArg* args, int numArgs)
{
    ...
    while (true)
    {
        ...
        if (*fmt == '\0')
        {
            ...
            {
                TINYFORMAT_ERROR(
                    "tinyformat: Not enough conversion specifiers in format "
                    "string");
            }
            break;
        }
        ...
        int ntrunc = -1;
        const char* fmtEnd =
            streamStateFromFormat(out, positionalMode, spacePadPositive, ntrunc,
                                  fmt, args, argIndex, numArgs);
