...
[[nodiscard]] static bool ParsePrechecks(const std::string& str)
{
    ...
    if (str.size() >= 1 &&
        (IsSpace(str[0]) ||
         IsSpace(str[str.size() - 1])))  // No padding allowed
        return false;
...
bool ParseUInt64(const std::string& str, uint64_t* out)
{
    if (!ParsePrechecks(str)) return false;
    if (str.size() >= 1 &&
        str[0] == '-')  // Reject negative values, unfortunately strtoull
                        // accepts these by default if they fit in the range
        return false;
    char* endp = nullptr;
...
bool TryHex(const std::string& str, std::vector<unsigned char>& rv)
{
    ...
    while (*psz)
    {
        ...
        signed char c = HexDigit(*psz++);
        if (c == (signed char)-1)
            return !psz[-1];  // IsSpace() trimmed off end and we ended up with
                              // a 'non hex' nul terminator
        unsigned char n = (c << 4);
        c = HexDigit(*psz++);
