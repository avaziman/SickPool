...
class CVerusHash
{
   public:
    ...
    static void (*haraka512Function)(unsigned char *out,
    ...
    void ExtraHash(unsigned char hash[32])
    {
...
class CVerusHashV2
{
   public:
    ...
    static void (*haraka512Function)(unsigned char *out,
                                     const unsigned char *in);
    static void (*haraka512KeyedFunction)(unsigned char *out,
                                          const unsigned char *in,
                                          const u128 *rc);
    static void (*haraka256Function)(unsigned char *out,
                                     const unsigned char *in);
    ...
    inline void ExtraHash(unsigned char hash[32])
    {
        (*haraka512Function)(hash, curBuf);
    }
    inline void ExtraHashKeyed(unsigned char hash[32], u128 *key)
    {
        (*haraka512KeyedFunction)(hash, curBuf, key);
    }
    ...
    static u128 *GenNewCLKey(unsigned char *seedBytes32)
    {
        ...
        {
            // generate a new key by chain hashing with Haraka256 from the last
            // curbuf
            int n256blks = size >> 5;
        ...
        memset((unsigned char *)key + (size + refreshsize), 0,
               size - refreshsize);
        return (u128 *)key;
    }
    ...
    void Finalize2b(unsigned char hash[32])
    {
        ...
        (*haraka512KeyedFunction)(hash, curBuf,
                                  key + IntermediateTo128Offset(intermediate));
    }

    inline unsigned char *CurBuffer() { return curBuf; }
