...
struct serializer
{
    ...
    {
        return serialize(ar, v, typename boost::is_integral<T>::type(),
                         typename is_blob_type<T>::type());
    }
    static bool serialize(Archive& ar, T& v, boost::false_type,
                          boost::true_type)
    {
        ar.serialize_blob(&v, sizeof(v));
        return true;
    }
    static bool serialize(Archive& ar, T& v, boost::true_type,
                          boost::false_type)
    {
        ar.serialize_int(v);
        return true;
...
#define PREPARE_CUSTOM_VECTOR_SERIALIZATION(size, vec)            \
    ::serialization::detail::prepare_custom_vector_serialization( \
        size, vec, typename Archive<W>::is_saving())
...
#define DEFINE_SERIALIZATION_VERSION(v) \
    inline static uint32_t get_serialization_version() { return v; }

#define VERSION_ENTRY(f)                                                \
    do                                                                  \
    {                                                                   \
        ar.tag(#f);                                                     \
        if (ar.is_saving_arch()) f = this->get_serialization_version(); \
        bool r = ::do_serialize(ar, f);                                 \
        if (!r || !ar.stream().good()) return false;                    \
    } while (0);
...
class serializable_pair : public std::pair<first_type, second_type>
{
   ...
   public:
    serializable_pair() {}
    serializable_pair(const first_type& a, const second_type& b)
        : std::pair<first_type, second_type>(a, b)
    {
    }
    serializable_pair(const serializable_pair& sp)
        : std::pair<first_type, second_type>(sp.first, sp.second)
    {
    }
...
template <typename first_type, typename second_type>
serializable_pair<first_type, second_type> make_serializable_pair(
    const first_type& first_value, const second_type& second_value)
{
    return serializable_pair<first_type, second_type>(first_value,
                                                      second_value);
}
...
{
template <typename T>
void prepare_custom_vector_serialization(
    size_t size, std::vector<T>& vec,
    const boost::mpl::bool_<true>& /*is_saving*/)
{
}
...
void prepare_custom_vector_serialization(
    size_t size, std::vector<T>& vec,
    const boost::mpl::bool_<false>& /*is_saving*/)
{
    vec.resize(size);
}
...
{
    return detail::do_check_stream_state(ar.stream(),
